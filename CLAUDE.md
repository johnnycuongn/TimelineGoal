...
- Rule 0: never read `.env.local`; never add a `sb_secret_` key anywhere. Rule 1: mobile-first, verify at 320/375/768/1280 before claiming a screen works. Rule 2: publishing is the owner's call — ask before `vercel --prod` or before pushing a `v*` tag (that tag is what fires the release workflow), use `vercel deploy --target=preview` to check a build, and remember that the *first* deployment of a fresh Vercel project is promoted to production by the CLI whatever you asked for.

...
- **Emoji render as [?] tofu boxes on the iOS 26.3.1 simulator** (charm picker, any emoji, even Safari) — **known Apple bug in the iOS 26.2–26.3.1 simulator runtimes** (Apple Color Emoji fails to load; font file is present in the image but fontd won't serve it). System-wide in the sim, NOT an app/RN/Expo bug; physical devices unaffected. Apple fixed it in the iOS 26.4 final runtime. Fix here: iOS 26.4.1 (23E254a) runtime installed alongside — note `xcodebuild -downloadPlatform iOS -buildVersion X` refuses non-matched versions unless you also pass `-exportPath` (then `xcodebuild -importPlatform <dmg>`). Refs: Apple forums thread 817957, XcodesOrg/xcodes#468.

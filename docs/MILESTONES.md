...
- **"No iOS devices available in Simulator.app"**: Xcode 26.3 is installed but shipped without an iOS simulator runtime (only a dead iOS 14.3 from an old Xcode). Fix: `xcodebuild -downloadPlatform iOS` (~8GB) — started 2026-07-11. Until it lands, use the Android emulator (Pixel_3a_XL_API_33): `npm start` → press `a`. Android reaches the Firebase emulators via `10.0.2.2` (handled in `src/lib/firebase.ts`).

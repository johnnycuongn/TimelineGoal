...
**Current Status**: ✅ **M0–M3 complete** (dev-local; M3 finished + live-verified single-phone 2026-07-13). **2026-07-15: realistic 3D pup (CC-BY bulldog-puppy scan, PBR + emotions) shipped to the Den toggle + `/frenchie-test` rig, live-verified on Android — iPhone feel-test pending (user).** Pending live checks: two-phone seal slam, M1/M2 two-user exit tests, offline check-ins. Open decision gate: 3D pup (now realistic) vs SVG Mochi (see M2 3D items). Next up: **M4 — Corners**. Stack is **Expo SDK 57**; cloud Auth/Firestore/Storage console enablement still deferred (all dev on emulators).

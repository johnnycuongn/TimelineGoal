...
**Current Status**: ✅ **M0–M3 complete** (dev-local; M3 finished + live-verified single-phone 2026-07-13). Pending live checks: two-phone seal slam, M1/M2 two-user exit tests, offline check-ins. Open decision gate: 3D pup vs SVG Mochi (see M2 3D item). Next up: **M4 — Corners**. Stack is **Expo SDK 57**; cloud Auth/Firestore/Storage console enablement still deferred (all dev on emulators).
